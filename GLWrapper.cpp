...
{
    log->MESSAGE(4, "GL call: glAttachShader(%d, %d)\n", shaderProgram, shader);
...
GLuint GLWrapper::GLCREATEPROGRAM(void)
{
    ...
    log->MESSAGE(4, "GL call: glCreateProgram() = %d\n", program);
    flushGLErrors();
    return program;
}
...
{
    log->MESSAGE(4, "GL call: glLinkProgram(%d)\n", program);
...
{
    log->MESSAGE(5, "GL call: glUseProgram(%d)\n", program);
...
{
    log->MESSAGE(5, "GL call: glViewPort(%d, %d, %d, %d)\n", x, y, width, height);
...
{
    log->MESSAGE(5, "GL call: glClear(0x%x)\n", mask);
    glClear(mask);
    flushGLErrors();
}
...
{
    log->MESSAGE(5, "GL call: glVertexAttribPointer(%d, %d, %d, %d, %d, %p)\n",
...
{
    log->MESSAGE(5, "GL call: glEnableVertexAttribPointer(%d)\n", index);
...
{
    log->MESSAGE(5, "GL call: glDrawArrays(%d, %d, %d)\n", mode, first, size);
...
{
    log->MESSAGE(4, "GL call: glGenTextures(%d, %p)\n", size, ptr);
...
{
    log->MESSAGE(5, "GL call: glBindTexture(%d, %d)\n", target, id);
...
{
    log->MESSAGE(4, "GL call: glPixelStorei(%d, %d)\n", type, align);
...
{
    log->MESSAGE(4, "GL call: glTexImage2D(%d, %d, %d, %d, %d, %d, %d, %d, %p)\n",
...
{
    log->MESSAGE(4, "GL call: glTexParameteri(%d, %d, %d)\n", target, pname, param);
...
GLint GLWrapper::GLGETATTRIBLOCATION(GLuint program, const GLchar *name)
{
    ...
    log->MESSAGE(4, "GL call: glGetAttribLocation(%d, '%s') = %d\n", program, name, rc);
    flushGLErrors();
    return rc;
}
...
GLint GLWrapper::GLGETUNIFORMLOCATION(GLuint program, const GLchar *name)
{
    ...
    log->MESSAGE(4, "GL call: glGetUniformLocation(%d, '%s') = %d\n", program, name, rc);
    flushGLErrors();
    return rc;
}
...
{
    log->MESSAGE(5, "GL call: glActiveTexture(%d)\n", texture);
...
{
    log->MESSAGE(5, "GL call: glUniform1i(%d, %d)\n", location, x);
...
{
    log->MESSAGE(5, "GL call: glUniform4f(%d, %f)\n", location, x);
...
{
    log->MESSAGE(5, "GL call: glUniform4f(%d, %f, %f, %f, %f)\n", location, x, y, z, w);
...
{
    log->MESSAGE(4, "GL call: glCompressedTexImage2D(0x%x, %d, 0x%x, %d, %d, %d, %d, %p)\n",
...
{
    log->MESSAGE(4, "GL call: glDrawElements(%d, %d, %d, %p)\n", mode, count, type, data);
...
{
    log->MESSAGE(5, "Attempting a call to eglSwapBuffers(%p, %p)\n", egl_display, egl_surface);
